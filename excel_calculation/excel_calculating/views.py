...
from .application.excel_calculation_service import (
    calculate_columns_statistics,
    calculate_columns_statistics_check_cell,
)
...
class ExcelCalculationViewCheckCells(APIView):
    parser_classes = [parsers.MultiPartParser]
    ...
    def post(self, request: Request) -> Response:
        ...
        uploaded_file = serializer.validated_data["file"]
        columns = serializer.validated_data["columns"]
        result = calculate_columns_statistics_check_cell(
            columns_names=columns, uploaded_file=uploaded_file
        )
        if serializer.is_valid():
            return Response(
